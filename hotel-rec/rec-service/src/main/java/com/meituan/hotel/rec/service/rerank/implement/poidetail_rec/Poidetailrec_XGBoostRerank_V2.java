 ...
 */
@Service("rerank-poidetailrec-xgboost-v2-service")
public class Poidetailrec_XGBoostRerank_V2 extends IReranker {

    @Resource
    private FeatureAssembler featureAssembler;

    private static Xgboost xgboost = new Xgboost();
    private static final Logger logger = RecUtils.getLogger(Poidetailrec_XGBoostRerank_V2.class.getSimpleName());
    private static Map<String, Integer> feature2IdxMap = new HashMap<String, Integer>();

    public static final String MODEL_FILE = "model/poidetail_rec/xgboost/xgboost_0606_logical";
    ...
    static {
        try {
            ...
            logger.info(RecUtils.getInfoString("load_poidetail_XGboost_Logical_Model_Done"));
        } catch (Exception e){
            logger.error(RecUtils.getErrorString("load_poidetail_XGboost_Logical_Model"), e);
        }
    }
    ...
    public RerankResponse rerank(RerankRequest request) {
        ...
        try {
            ...
            for (Integer poiId : poiIdsFeatureMap.keySet()) {
                ...
                double scorePredict = xgboost.predict(tuple, true);
